...
function SignInInner() {
  ...
  const [providers, setProviders] = useState<{ google: { enabled: boolean; label: string } } | null>(null);
  ...
  async function handleCredentialsSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      // Connect our credentials form directly to the devBypass endpoint so it's 100% active and works!
      const name = email.split("@")[0].split(".")[0];
      const capitalizedName = name.charAt(0).toUpperCase() + name.slice(1);
      await api.auth.devBypass(email, capitalizedName);
        ...
        >
          ...
          {error && (
            ...
            >
              ...
              <div className="text-micro text-[#ff4d6d]">
                {error === "google_oauth_not_configured"
                  ? "Google OAuth keys are not set. Try entering any Email & Password for instant Dev access!"
          ...
          <form onSubmit={handleCredentialsSubmit} className="space-y-4">
            ...
            <div className="flex items-center justify-between text-micro font-medium pt-1">
              <label className="flex items-center gap-2 cursor-pointer select-none text-[var(--color-slate)] hover:text-white transition-colors">
                <input
                  type="checkbox"
                  checked={rememberMe}
                  onChange={(e) => setRememberMe(e.target.checked)}
                  className="rounded border-[rgba(255,255,255,0.1)] bg-zinc-950 text-purple-600 focus:ring-0 cursor-pointer"
                />
                <span>Remember me</span>
              </label>
              <a href="#forgot" className="text-[var(--color-slate)] hover:text-white transition-colors">
                Forgot password?
              </a>
            </div>
            ...
            <Button
              ...
              variant="glow"
              disabled={loading}
              className="w-full h-11 gap-1.5 font-bold tracking-tight rounded-xl bg-gradient-to-r from-indigo-600 via-[#a24bff] to-purple-600 text-white cursor-pointer mt-2 text-label"
          ...
          <p className="mt-8 text-center text-body-sm font-medium text-[var(--color-slate)]">
            ...
            <Link href="/sign-up" className="text-purple-400 hover:text-purple-300 font-semibold inline-flex items-center gap-0.5">
              <span>Sign up</span>
